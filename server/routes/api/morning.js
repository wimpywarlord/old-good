        ...
        user.find({}, async function (err, users) {
            if (err) {
                ...
                for (let i = 0; i < users.length; i++) {
                    if (users[i].days <= 14) {
                        everyday_mailingList.push([
                            users[i].email,
                            users[i].name_of_sender,
                        ]);
                        ...
                        user.findOneAndDelete({ _id: users[i]._id }, function (
                            err,
                            res
                        ) {
                            if (err) {
...
fetchAllEmails().then((everyday_mailingList) => {
    ...
    everyday_mailingList.forEach((element) => {
        ...
        var mailOptions = {
            ...
            to: element[0],
            subject: `${element[1]} says "Goodmorning ${random_nickname} ${random_emoji}"`,
